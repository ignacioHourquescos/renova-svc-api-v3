...
app.post("/generalValidateUser/", controller.generalValidateUser);
	...
	con.query(sql, function (error, result, fields) {
		...
		const canti = data
			.reduce((acumulator, current) => {
				return acumulator + current.canti_kilos;
			}, 0)
			.toFixed(0);
		const impor = data
			.reduce((acumulator, current) => {
				return acumulator + current.impor;
			}, 0)
			.toFixed(0);
		const costo = data
			.reduce((acumulator, current) => {
				return acumulator + current.costo;
			}, 0)
			.toFixed(0);
